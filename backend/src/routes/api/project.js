...
router.get('/', verify, async (req, res) => {
  try {
    const {
      limit, tags, offset, sort, project
    } = req.query;
    const user = mongoose.Types.ObjectId(req.user.id);

    const all = await ProjectManager.get(
      user,
      { tags, id: project },
      sort,
      limit,
...
router.get('/list', async (req, res) => {
  try {
    const {
      email, tags, limit, offset, sort, project
    } = req.query;
...
router.post('/add', postRules, validate, verify, async (req, res) => {
  try {
    const {
      name, intro, link, tags, thumbnail
    } = req.body;
    const { user } = req;
    const created = await ProjectManager.create({
      name,
      intro,
      link,
      tags,
...
router.patch('/update', validate, verify, async (req, res) => {
  try {
    const {
      id, name, intro, tags, link, thumbnail
    } = req.body;
    const updated = await ProjectManager.update({
      id,
      name,
      intro,
      tags,
