...
const { invalidProfile, validProfile, qs } = require('../__mocks__/ProfileMock');
...
describe('Post Set Test Suit', async () => {
  ...
  it('Create a profile for non autheticated user', async () => {
    const res = await request(app).post('/api/profile/add').send({ ...validProfile, skills: skillIds[0] });
