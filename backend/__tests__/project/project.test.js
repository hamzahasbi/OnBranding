...
const {validProject, invalidProject, qs} = require('../__mocks__/ProjectMock');
...
describe('Project Set Test Suit', async () => {
    ...
    before(async () => {
        try {
            ...
            const project = await request(app).post('/api/project/add').auth(token, {type: 'bearer'}).send(validProject);
            projectIds.push(project.body.ressource._id);
            const project_2 = await request(app).post('/api/project/add').auth(token, {type: 'bearer'}).send(validProject);
    ...
    after(async () => {
        try {
            ...
            await Promise.all(projectIds.map(async (el) => {
    ...
    it('Create a Project for authenticated user', async () => {
        ...
        const res = await request(app).post('/api/project/add').auth(token, {type: 'bearer'}).send(validProject);
    ...
    it('Create a non Valid Project', async () => {
        ...
        const res = await request(app).post('/api/project/add').auth(token, {type: 'bearer'}).send(invalidProject);
    ...
    it('Update a Project with valid data', async () => {
        ...
        const res = await request(app).patch('/api/project/update').auth(token, {type: 'bearer'}).send({id: projectIds[0], tags: skillIds.join(',')});
        expect(res.status).to.eq(201);
        expect(res.body).to.haveOwnProperty('ressource');
    ...
    it('Remove a Project', async () => {
        ...
        const res = await request(app).delete('/api/project/remove').auth(token, {type: 'bearer'}).send({id: willBeRemoved});
    ...
    it('Get list of all authenticated user\'s projects with no query params', async () => {
        ...
        const res = await request(app).get('/api/project').auth(token, {type: 'bearer'});
        expect(res.status).to.eq(200);
        expect(res.body).to.haveOwnProperty('ressource');
    ...
    it('Get list of all authenticated user\'s projects with query params & no filter', async () => {
        ...
        const res = await request(app).get('/api/project').auth(token, {type: 'bearer'}).query(qs);
        expect(res.status).to.eq(200);
        expect(res.body).to.haveOwnProperty('ressource');
        const ressources = res.body.ressource;
        const count = res.body.count;
    ...
    it('Get list of all user\'s projects with no query params by email', async () => {
        ...
        const res = await request(app).get('/api/project/list').query({email: UserRegister.valid.email});
        expect(res.status).to.eq(200);
        expect(res.body).to.haveOwnProperty('ressource');
        const ressources = res.body.ressource;
        const count = res.body.count;
    ...
    it('Get list of all user\'s projects with query params & filter by email', async () => {
        ...
        const res = await request(app).get('/api/project/list').query({...qs, email: UserRegister.valid.email, tags: '6011ae06739fa71dbf34d892'});
        expect(res.status).to.eq(200);
        expect(res.body).to.haveOwnProperty('ressource');
        const ressources = res.body.ressource;
        const count = res.body.count;
    ...
    it('Get list of all user\'s projects with query params & filter by email', async () => {
        ...
        const res = await request(app).get('/api/project/list').query({...qs, email: UserRegister.valid.email, offset: 0, tags: skillIds[0].toString()});
        expect(res.status).to.eq(200);
        expect(res.body).to.haveOwnProperty('ressource');
        const ressources = res.body.ressource;
        const count = res.body.count;
    ...
    it('Get Project by id', async () => {
        ...
        const res = await request(app).get('/api/project/list').query({email: UserRegister.valid.email, project: projectIds[0].toString()});
        expect(res.status).to.eq(200);
        expect(res.body).to.haveOwnProperty('ressource');
        const ressources = res.body.ressource;
        const count = res.body.count;
